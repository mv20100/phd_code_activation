...
self.camera.start_live_video(None)
...
def update():
    ...
    if ui.rawRadio.isChecked():
        ui.rawImg.img.setImage(cam.image_array()[:,:,0:3],lut=useLut, levels=useScale)
        # ui.rawImg.setImage(data[ptr%data.shape[0]], lut=useLut, levels=useScale)
        ui.stack.setCurrentIndex(1)
    elif ui.rawGLRadio.isChecked():
        ui.rawGLImg.setImage(cam.image_array()[:,:,0:3], lut=useLut, levels=useScale)
        # ui.rawGLImg.setImage(data[ptr%data.shape[0]], lut=useLut, levels=useScale)
        ui.stack.setCurrentIndex(2)
    else:
        img.setImage(cam.image_array()[:,:,0:3], autoLevels=False, levels=useScale, lut=useLut, autoDownsample=downsample)
